    ...
    <div className="bg-[#f5f0e8]">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {residents.map(horse => <HorseCard key={horse.id} horse={horse} />)}
        </div>
      </section>
      ...
      <section className="bg-[#ede5d4] py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {rehoming.map(horse => <HorseCard key={horse.id} horse={horse} />)}
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-4">
          <Link to="/adoption" className="inline-flex items-center gap-2 px-7 py-3 bg-[#b8922a] text-white text-sm font-medium rounded-full hover:bg-[#8c6e1e] transition-colors cursor-pointer">
            Adoption enquiry <ArrowRight size={16} />
          </Link>
          <Link to="/sponsorship" className="inline-flex items-center gap-2 px-7 py-3 border border-[#1a1a18] text-[#1a1a18] text-sm font-medium rounded-full hover:bg-[#1a1a18] hover:text-[#f5f0e8] transition-colors cursor-pointer">
            Sponsor a horse
          </Link>
        </div>
      </section>
...
function HorseCard({ horse }: { horse: (typeof HORSES)[0] }) {
  return (
    <Link to={`/horses/${horse.id}`} className="bg-white rounded-2xl group block overflow-hidden shadow-sm border border-[#ddd4be]/50 cursor-pointer">
      <div className="relative aspect-[4/5] sm:aspect-[3/2] overflow-hidden bg-[#ede5d4]">
        <img src={horse.image} alt={horse.name} loading="lazy" className="h-full w-full object-cover transition-transform duration-[900ms] ease-out group-hover:scale-105" />
      ...
      <div className="p-6">
        <div className="flex items-start justify-between gap-4">
          <h3 className="font-serif text-2xl leading-tight text-[#1a1a18]">{horse.name}</h3>
