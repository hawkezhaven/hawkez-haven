...
function HorseCard({ horse }: { horse: (typeof HORSES)[0] }) {
  return (
    <Link to={`/horses/${horse.id}`} className="bg-white rounded-2xl group block overflow-hidden shadow-sm border border-[#ddd4be]/50 cursor-pointer">
      <div className="relative overflow-hidden bg-[#ede5d4]">
        <img src={horse.image} alt={horse.name} loading="lazy" width={1200} height={800} style={{ aspectRatio: "auto 3 / 2" }} className="h-auto w-full object-contain object-center transition-transform duration-[900ms] ease-out group-hover:scale-105" />
